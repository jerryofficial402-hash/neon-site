...
print("--- SEO AUDIT FOR NY MAIN STATE PAGE ---")
...
if h1s:
    print(f"H1 Text: {h1s[0].text.strip()}")

# 3. Breadcrumbs
breadcrumb_nav = soup.find('nav', attrs={'aria-label': 'Breadcrumb'})
print(f"Breadcrumb HTML Present: {breadcrumb_nav is not None}")
...
print(f"Links to NY cities page: {sum(1 for l in internal_links if 'new-york-car-shipping-cities' in l)}")

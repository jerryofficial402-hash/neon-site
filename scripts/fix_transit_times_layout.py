...
# Read master header & footer from index.html
with open(INDEX_FILE, "r", encoding="utf-8") as f:
    hp_content = f.read()

# Extract master header
header_start = hp_content.find('<!-- Global Header -->')
header_end = hp_content.find('<!-- Hero Section -->')
if header_end == -1:
    header_end = hp_content.find('</header>') + len('</header>')
master_header = hp_content[header_start:header_end]

# Extract master footer
...
# Update master footer carrier claim if old version
...
<head>
  ...
  <!-- Tailwind CSS Engine for Dynamic Subpage Utility Classes -->
...
print("SUCCESS: Restored Tailwind CDN engine and clean header/footer layouts to /car-shipping-transit-times/index.html")

...
# We will use Dallas as the base template since it has the most updated design (balanced hero, nice routing, etc)
...
for city in cities_data:
    # We will do string replacement on the Dallas HTML so we don't mess up BS4 formatting
    html = template_html
    ...
    # 3. Main Body Context
    # Dallas has a section "Why Dallas Auto Transport Works Differently"
    ...
    # 5. Common Routes List
    # Find the UL containing the routes
    ...
    # Save to both .html file and /index.html inside folder just to ensure compatibility with their host
    html_file = os.path.join(base_dir, f"{city['slug']}.html")
    with open(html_file, "w", encoding="utf-8") as f:
        f.write(html)
        
    dir_path = os.path.join(base_dir, city['slug'])
